...
void ledcAttachPin(uint8_t pin, uint8_t chan)
{
    ...
    e = ledc_channel_config(&ledc_channel);
    if(e != ESP_OK){
        log_e("ledc_channel_config failed with error %04X\n", e);
    }
}
...
static int8_t pin_to_channel[SOC_GPIO_PIN_COUNT];
static int8_t channel_to_pin[LEDC_CHANNELS];

void exp32_hal_led_init(void){
    for(uint8_t channel_idx = 0; channel_idx < LEDC_CHANNELS; channel_idx++)
        channel_to_pin[channel_idx] = -1;
    for(uint8_t pin_idx = 0; pin_idx < SOC_GPIO_PIN_COUNT; pin_idx++)
        pin_to_channel[pin_idx] = -1;
}
static uint8_t analog_resolution = 5;
static int cnt_channel = LEDC_CHANNELS;
// static int analog_frequency = 100000;
static int analog_frequency = 2000000;

int get_free_pwm_channel(void){
    // log_v("getting free pin\n");
    for(uint8_t channel_idx = 0; channel_idx < LEDC_CHANNELS; channel_idx++){
        if(channel_to_pin[channel_idx] == -1){
            // log_v("channel_to_pin[%d] == %d\n", channel_idx, channel_to_pin[channel_idx]);
            return channel_idx;
        }
    }
    return -1;
...
void freePWM(uint8_t pin){
    ...
    // Free in pool of PWMs
    channel_to_pin[channel] = -1;

    ledcDetachPin(pin);

    pin_to_channel[pin] = -1;
    cnt_channel++;
}
